...
// Страховка, оплачиваемая ежемесячно (например, ипотечное страхование жизни/объекта — банк
// поднимает ставку, если пропустить взнос), напоминается каждый месяц в день оплаты.
// Страховка с ежегодной оплатой (ОСАГО/КАСКО и т.п.) напоминается раз в год за 30 дней до
// окончания — оба напоминания через настоящие повторяющиеся триггеры, а не разовые, чтобы
// не пропасть после первого же года/месяца.
